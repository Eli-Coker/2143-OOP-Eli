...
class PeopleDB(JsonDB):
    ...
    def delete_person(self, username):
        ...
        if not username:
            raise ValueError("Username must be provided for deletion.")
    ...
    def update_username(self, current_username, new_username):
        ...
        if not current_username or not new_username:
            raise ValueError("Both current and new usernames must be provided.")
        
        # Check if the new username is already taken
        if any(record.get("user", {}).get("username") == new_username for record in self.data):
